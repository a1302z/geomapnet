...
class AachenDayNight(data.Dataset):
    ...
    def __init__(self, data_path, train, train_split=0.9#5./6.
                 , overfit=None,
        ...
        transform=identity, 
        semantic_colorized_transform=identity, target_transform=identity, scene='', concatenate_inputs=False):
        """
        np.random.seed(seed)
        ...
        if not files_exist:
            ...
            if abs(train_split-train_split_read) > 1e-3:
                ...
                if print_debugging_info:
                    print('Read specified keys for training and validation (Split: %.2f)'%self.train_split)
                self.train_keys = [int(x) for x in train_lines[1:]]
                train_file.close()
                test_file = open(os.path.join(data_path,'test_'+str(train_split)+'.txt'), 'r')
    ...
    def _split_dataset(self, train_split, data_path):
        ...
        self.test_keys.sort()
        train_file = open(os.path.join(data_path+ 'train_'+str(train_split)+'.txt'), 'w+')
        train_file.write(str(train_split)+"\n")
        for p in self.train_keys:
            train_file.write(str(p)+'\n')
        train_file.close()
        test_file = open(os.path.join(data_path+'test_'+str(train_split)+'.txt'), 'w+')
    ...
    def print_point(self, index):
        print(str(self.points[self.train_keys[index] if self.train else self.test_keys[index]]))
...
def main():
    ...
    for i in loader.test_keys:
        if i in loader.train_keys:
            print('ERROR1')
    for i in loader.train_keys:
        if i in loader.test_keys:
            print('ERROR2')
            
    loader = AachenDayNight('../data/deepslam_data/AachenDayNight/', True, train_split=5./6.)
