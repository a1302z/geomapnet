...
class VidLoc(nn.Module):
        ...
        for layer in range(n_layers):
            for connection in ['ih', 'hh']:
                for suffix in ['', '_reverse']:
                    bias_name = 'bias_{:s}_l{:d}{:s}'.format(
                        connection, layer, suffix)
    ...
    def forward(self, x, cuda=False, async=True):
        ...
        x, (self.fc_h, self.fc_c) = self.lstm_fc(
            x, (self.fc_h, self.fc_c))  # B x G x d'
    ...
    def reset_hidden_states(self, batch_size):
        self.fc_h.data.resize_(
            2, batch_size, self.lstm_fc.hidden_size).normal_()
        self.fc_c.data.resize_(
            2, batch_size, self.lstm_fc.hidden_size).normal_()
        self.xyz_h.data.resize_(
            1, batch_size, self.lstm_xyz.hidden_size).normal_()
        self.xyz_c.data.resize_(
            1, batch_size, self.lstm_xyz.hidden_size).normal_()
        self.wpqr_h.data.resize_(
            1, batch_size, self.lstm_wpqr.hidden_size).normal_()
        self.wpqr_c.data.resize_(
            1, batch_size, self.lstm_wpqr.hidden_size).normal_()
